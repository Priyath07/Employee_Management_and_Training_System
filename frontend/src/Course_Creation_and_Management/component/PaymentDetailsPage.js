...
function PaymentDetailsPage() {
    ...
    useEffect(() => {
        const fetchPaymentDetails = async () => {
            try {
                const response = await axios.get('http://localhost:8050/Course_Creation_and_Management/CoursePayment/coursePaymentAll');
    ...
    return (
        <><div>
            ...
            <button className="btn btn-warning" onClick={generatePDF} style={PDFButtonStyle}>Generate PDF Report </button>

            <Link to={`/course`} className="btn btn-dark">
                Back
            </Link>
